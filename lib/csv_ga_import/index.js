    ...
    return low(adapt).then((db) => {
        ...
        return csv().fromFile(item.path).then((data) => {
            ...
            rowLoop: while (rowI < rowLen) {
				...
				let days = db.get('days');
				if(days.find(query).value()){
					
					days.assign(obj).write();
                ...
                //console.log(db.get('days').find(query));
    ...
    return new Promise((resolve, reject) => {
        ...
        .on('data', (item) => {
            ...
            cvsToJson(options, item).then(() => {
                ...
                console.log(item.reportType + ' csv file processed');
